...
with open(r"C:\Users\ASUS\Documents\GUVI ZEN CLASSES\MAINT BOOT\Crime Analyser\crime_analyser\model.pkl","rb") as file:
...
<style>
    ...
    with col1:
         ...
         domestic = st.selectbox('Domestic',domestic_list,index=None)
    ...
    #st.write("Encoded Inputs:", description,domestic, district, location)
    ...
    if st.button(':violet[Predict]',use_container_width=True):
        ...
        if None in [description,domestic,district,location]:
            ...
            domestic = domestic_list_encoded[domestic]

            district = district_list_encoded[district]

            #st.write("Encoded Description:", description)
            #st.write("Encoded Location:", location)
            #st.write("Encoded Domestic:", domestic)
            #st.write("Encoded District:", district)'''
            

            def get_prediction(description,domestic,district,location):
                arr = np.array([description,domestic,district,location])
                input_data = arr.reshape(1,4)
                #st.write(f"Encoded Input Data: {input_data.tolist()}")
                prediction =class_model.predict(input_data)
                #st.write(f"Raw Model Prediction: {prediction}")
                pred = prediction.tolist()
                #st.write(f"Raw Model Prediction_list: {pred}")
                pred_1 = int(pred[0])
                #st.write(f"pred: {pred_1}")
                pred_new = reversed_prime_type[pred_1]
                #st.write(f"Mapped Crime Type: {pred_new}")
                return pred_new           


            prediction = get_prediction(description,domestic,district,location)

            

            st.subheader((f":green[Crime_Type :] {prediction}"))

    image = Image.open('police_name_board.jpg')
